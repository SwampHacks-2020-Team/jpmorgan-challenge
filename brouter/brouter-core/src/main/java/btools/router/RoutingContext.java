...
public final class RoutingContext
{
    ...
    public int calcDistance( int lon1, int lat1, int lon2, int lat2, short ele1, short ele2 )
    {
        ...
        double rele1 = ele1 / 4.0;
        double rele2 = ele2 / 4.0;

        if((rele1 + rele2) / 2 > maxAltitude) {
            nogoCost = -1;
            System.out.println("Zeroed cost for elev pair: " + ele1 + ", " + ele2 + " -> " + rele1 + ", " + rele2);
        }

        //System.out.println("Elevation pair: " + ele1 + ", " + ele2 + " -> " + rele1 + ", " + rele2);
        ...
        /*System.out.println("Before custom, nogo cost was: " + nogoCost);

        //System.out.println("lon1: " + lon1 + ", lon2: " + lon2 + ", lat1: " + lat1 + ", lat2: " + lat2);

        double rlon1 = (lon1 - 0.5) / 1000000. - 180;
        double rlon2 = (lon2 - 0.5) / 1000000. - 180;
        double rlat1 = (lat1 - 0.5) / 1000000. - 90;
        double rlat2 = (lat2 - 0.5) / 1000000. - 90;
        System.out.println("lon1: " + rlon1 + " lat1: " + rlat1 + "lon2: " + rlon2 + " lat2: " + rlat2);

        */
/*
        double a = 0.0002;
        double udxr = rlon2 - rlon1;
        double udyr = rlat2 - rlat1;
        double difx = udxr / (Math.sqrt(udxr*udxr + udyr*udyr) + 1E-7);
        double dify = udyr / (Math.sqrt(udxr*udxr + udyr*udyr) + 1E-7);
        double out1x = rlon1 - difx * a;
        double out1y = rlat1 - dify * a;
        double out2x = rlon2 + difx * a;
        double out2y = rlat2 + dify * a;
        double orthoux = 1;
        double orthouy = -1/(dify/(difx + 1E-7) + 1E-7);
        double orthox = orthoux / (Math.sqrt(orthoux*orthoux + orthouy*orthouy) + 1E-7);
        double orthoy = orthouy / (Math.sqrt(orthoux*orthoux + orthouy*orthouy) + 1E-7);
        double rect11x = out1x - orthox * a;
        double rect11y = out1y - orthoy * a;
        double rect12x = out1x + orthox * a;
        double rect12y = out1y + orthoy * a;
        double rect21x = out2x - orthox * a;
        double rect21y = out2y - orthoy * a;
        double rect22x = out2x + orthox * a;
        double rect22y = out2y + orthoy * a;

        System.out.println("lon1: " + rlon1 + " lat1: " + rlat1 + "lon2: " + rlon2 + " lat2: " + rlat2);
        System.out.println("rect: [(" + rect11x + "," + rect11y + "), (" + rect12x + "," + rect12y + "), " +
                            "(" + rect21x + "," + rect21y + "), (" + rect22x + "," + rect22y + ")]");

        Position rect11 = new Position(rect11x, rect11y);
        Position rect12 = new Position(rect12x, rect12y);
        Position rect21 = new Position(rect21x, rect21y);
        Position rect22 = new Position(rect22x, rect22y);
        List<Position> positions = new ArrayList<Position>();
        positions.add(rect11);
        positions.add(rect12);
        positions.add(rect22);
        positions.add(rect21);
        positions.add(rect11);
...
/*
        ...
        double udxr = rlon2 - rlon1;
        double udyr = rlat2 - rlat1;
        double dist = Math.sqrt(udxr*udxr + udyr*udyr);
        if(dist == 0) return (int)(d + 1.0);
        double vx = udxr / dist * a;
        double vy = udyr / dist * a;

        double incamt = a / dist;

        //List<Bson> queries = new ArrayList<Bson>((int) (dist/a) + 2);
        ...
        try {
            ...
            AltMongo amon = AltMongo.altMongo;
            MongoCollection<Document> coordscol = amon.getCoords();

            /*

            for(double offsetamt = 0; offsetamt <= dist + 1E-8; offsetamt += incamt) {
                double px = rlon1 + vx * offsetamt;
                double py = rlat1 + vy * offsetamt;

                Bson nearfilt = Filters.geoWithinCenterSphere("loc", px, py, 10.0);
                //Bson nearfilt = Filters.nearSphere("loc", px, py, 10.0, 0.0);
                Bson gtfilt = Filters.gt("alt", 30);
                Bson together = Filters.and(nearfilt, gtfilt);
                //FindIterable<Document> result = coordscol.countDocuments(together);//.limit(5);
                //Iterator<Document> itr = result.iterator();
                //long count = coordscol.countDocuments(together);
                //coordscol.count(together);
                //if(coordscol.find(together).first() != null) {//itr.hasNext()) {
                if(coordscol.countDocuments(together) != 0) {
                    nogoCost = -1;
                    System.out.println("Set custom nogo, ");
                    break;
                }
            }*/
...
/*
            ...
            //Bson nearfilt = Filters.geoWithin("loc", polygon);
            //Bson nearfilt = Filters.nearSphere("loc", )
            Bson nearfilt = Filters.geoWithinCenterSphere("loc", rlon1, rlat1, 10.0);
            Bson gtfilt = Filters.gt("alt", 35);
            Bson together = Filters.and(nearfilt, gtfilt);

            long count = coordscol.countDocuments(together);
            //FindIterable<Document> altnogopoints = coordscol.find(together);


      //if(altnogopoints.iterator().hasNext()) {
            if(count != 0) {
                nogoCost = -1;
                System.out.println("Set custom nogo");
           }
            ...
            //System.out.println("ok  " + Thread.currentThread().getId());
        } catch(Exception ex) {
            ex.printStackTrace();
            //System.out.println("ded " + Thread.currentThread().getId());
        }

        System.out.println("After custom, nogo cost was: " + nogoCost);
*/
